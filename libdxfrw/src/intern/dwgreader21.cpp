...
bool dwgReader21::parseSysPage(duint64 sizeCompressed, duint64 sizeUncompressed, duint64 correctionFactor, duint64 offset, duint8* decompData) {
  ...
  dwgCompressor::decompress21(tmpDataRS, decompData, static_cast<duint32>(sizeCompressed), static_cast<duint32>(sizeUncompressed));
  delete[]tmpDataRaw;
  delete[]tmpDataRS;
  return true;
}
...
bool dwgReader21::parseDataPage(dwgSectionInfo si, duint8* dData) {
  ...
  for (std::map<duint32, dwgPageInfo>::iterator it = si.pages.begin(); it != si.pages.end(); ++it) {
    ...
    dwgCompressor::decompress21(tmpPageRS, pageData, static_cast<duint32>(pi.cSize), static_cast<duint32>(pi.uSize));
...
bool dwgReader21::readFileHeader() {
  ...
  while (PagesMapSizeUncompressed > i) {
    ...
    sectionPageMapTmp[ind] = dwgPageInfo(ind, address, static_cast<duint32>(size));
    address += size;
