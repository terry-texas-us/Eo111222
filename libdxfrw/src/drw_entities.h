...
public:
  ...
  DRW_Entity() : eType(DRW::UNKNOWN), handle(DRW::NoHandle), parentHandle(DRW::NoHandle), appData(0),
    ...
    extAxisY(DRW_Coord()), curr(nullptr) {
  }
...
public:
  ...
  ~DRW_LWPolyline() {
    for (DRW_Vertex2D* vert : vertlist) {
      delete vert;
    }
    vertlist.clear();
...
public:
  ...
  ~DRW_Spline() {
    for (DRW_Coord* pt : controllist) {
      delete pt;
    }
    controllist.clear();
    for (DRW_Coord* pt : fitlist) {
      delete pt;
    }
    fitlist.clear();
