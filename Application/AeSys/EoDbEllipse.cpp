 ...
 */
EoDbEllipse::EoDbEllipse(const EoGePoint3d& center,
...
EoDbEllipse::EoDbEllipse(EoGePoint3d& center,
    EoGeVector3d& normal,
    double radius,
    std::int16_t color,
...
EoDbEllipse::EoDbEllipse(EoGePoint3d start, EoGePoint3d intermediate, EoGePoint3d end) {
  ...
  if (Eo::IsGeometricallyNonZero(determinant)) {  // Three points are not colinear
    ...
    if (Eo::IsGeometricallyNonZero(dAng[1] - dMax) && Eo::IsGeometricallyNonZero(dAng[1] - dMin)) {
      ...
      const EoGePoint3d ptRot = start.RotateAboutAxis(m_center, normal, Eo::HalfPi);
...
void EoDbEllipse::AddToTreeViewControl(HWND tree, HTREEITEM parent) {
  tvAddItem(tree, parent, L"<Ellipse>", this);
}
...
void EoDbEllipse::CutAt2Points(const EoGePoint3d& firstPoint,
...
void EoDbEllipse::GenPts(EoGePoint3d centerPoint,
    EoGeVector3d majorAxis,
    EoGeVector3d minorAxis,
...
EoGePoint3d EoDbEllipse::PointAtStartAngle() {
...
void EoDbEllipse::GetExtents(AeSysView* view,
    EoGePoint3d& ptMin,
    EoGePoint3d& ptMax,
...
int EoDbEllipse::IsWithinArea(const EoGePoint3d& lowerLeft, const EoGePoint3d& upperRight, EoGePoint3d* ptInt) {
  ...
  const double dRad = EoGeVector3d(m_center, ptBeg).Length();
...
EoGePoint3d EoDbEllipse::GoToNextControlPoint() {
  const double angle = (sm_RelationshipOfPoint < Eo::geometricTolerance) ? m_sweepAngle : 0.0;
...
void EoDbEllipse::GetBoundingBox(EoGePoint3dArray& ptsBox) const {
  ...
  if (m_sweepAngle < 3. * Eo::TwoPi / 4.0) {
    const double dEndX = std::cos(m_sweepAngle);
    const double dEndY = std::sin(m_sweepAngle);
