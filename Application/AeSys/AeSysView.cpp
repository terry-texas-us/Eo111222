...
ON_COMMAND(ID_INSERT_TRACING, OnInsertTracing)
...
ON_COMMAND(ID_SHEETSETUP_FORMFACTOR, OnSheetSetupFormFactor)
...
// ON_COMMAND_RANGE for draw mode commands is handled by DrawModeState when active.
#pragma warning(pop)
...
ON_COMMAND(ID_DRAW_MODE_UNDO_LAST, &AeSysView::OnDrawModeUndoLast)
ON_COMMAND(ID_DRAW_MODE_FINISH, &AeSysView::OnDrawModeFinish)
ON_COMMAND(ID_DRAW_MODE_POLYLINE, &AeSysView::OnDrawModeFinishPolyline)
