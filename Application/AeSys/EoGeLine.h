...
class EoGeLine {
  ...
  */
  ...
  /** @brief Determines if the line segment is wholly or partially contained within a specified rectangular window in
   * the XY plane.
  ...
  */
   ...
   * @note A positive perpendicular projection distance results in a point to the left of the direction vector defined
   * by the two points. Projected point is undefined if the begin and end points coincide.
   */
   ...
   * @return A 3D point located at the specified distance from the end point along the direction toward the beginning
   * point.
   */
   ...
   * @return A 3D point located at the specified distance from the beginning point along the direction toward the end
   * point.
   */
  ...
  /**
   ...
   @param pointParametricRelationship Output parameter that receives the parametric relationship value of the point
  ...
  * @return The angle in radians between the two lines when projected onto the XY plane, or 0.0 if either line has
  near-zero length.
  * @note
  *	Angle is found using the inner product.
  * @code
    Angle = acos ( [v1 dot v2] /  [len(v1) * len(v2)] )
    Angle is between 0 and 2π. If angle is 0 lines are in same direction and if angle is π lines are in opposite
  direction. To get acute angle, all angles greater than half π must be subtracted from π.
    @endcode
  */
   ...
   * @return The intersection point of the line with the plane. If the line is parallel to the plane, returns the begin
   * point.
