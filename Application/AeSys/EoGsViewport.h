...
class EoGsViewport {
   ...
   * @note The function maps NDC coordinates in the range [-1, 1] to client coordinates in the range [0, width] for x
   * and [0, height] for y. The y-coordinate is inverted to account for the difference in coordinate systems between NDC
 ...
 public:  // Constructors and destructors
   ...
   * @param pointsArray An array of EoGePoint4d objects representing the 3D points in homogeneous coordinates to be
   * projected.
   * @note Each point in pointsArray is expected to be in homogeneous coordinates (x, y, z, w). The function divides
   * each point by its w component to convert it to normalized device coordinates before projecting it to screen space.
   */
   ...
   * @note This function returns the physical width of the viewport based on the device's characteristics, which may
   * differ from the logical width used for projection calculations.
   */
