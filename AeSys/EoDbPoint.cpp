...
EoDbPoint::EoDbPoint(const EoGePoint3d& point)
: m_Point(point) {
	m_PenColor = 1;
	m_PointStyle = 1;
	m_NumberOfDatums = 0;
...
EoDbPoint::EoDbPoint(EoInt16 penColor, EoInt16 pointStyle, const EoGePoint3d& point)
: m_Point(point) {
	m_PenColor = penColor;
...
EoDbPoint::EoDbPoint(EoInt16 penColor, EoInt16 pointStyle, const EoGePoint3d& point, EoUInt16 numberOfDatums, double* data)
: m_Point(point) {
	m_PenColor = penColor;
	m_PointStyle = pointStyle;
...
void EoDbPoint::AddToTreeViewControl(HWND hTree, HTREEITEM hParent) {
	tvAddItem(hTree, hParent, L"<Point>", this);
}
			...
			for (int Row = - 2; Row <= 2; Row++) {
				for (int Col = - 2; Col <= 2; Col++) {
					if (abs(Row) == 2 || abs(Col) == 2) {
						deviceContext->SetPixel(pnt.x + Col, pnt.y + Row, HotPenColor);
					}
				}
			}
...
void EoDbPoint::FormatExtra(CString& str) {
	str.Format(L"Color;%s\tStyle;%d", FormatPenColor(), m_PointStyle);
}
void EoDbPoint::FormatGeometry(CString& str) {
	str += L"Point;" + m_Point.ToString();
}
EoGePoint3d EoDbPoint::GetCtrlPt() {
	return (m_Point);
}
...
void EoDbPoint::Transform(EoGeTransformMatrix& tm) {
	m_Point = tm * m_Point;
}
