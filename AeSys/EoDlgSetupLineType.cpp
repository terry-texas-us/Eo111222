...
EoDlgSetupLineType::EoDlgSetupLineType(CWnd* pParent /*=NULL*/) :
	CDialog(EoDlgSetupLineType::IDD, pParent) {
}
	...
	if (controlIdentifier == IDC_LINETYPES_LIST_CONTROL) {
		...
		case ODA_DRAWENTIRE: {
				...
				if(Item != - 1) {
					...
					float UExtent = static_cast<float>(SubItemRectangle.right + SubItemRectangle.left) / static_cast<float>(DeviceContext.GetDeviceCaps(LOGPIXELSX));
					float VExtent = static_cast<float>(SubItemRectangle.bottom + SubItemRectangle.top) / static_cast<float>(DeviceContext.GetDeviceCaps(LOGPIXELSY));
					...
					float UMin = static_cast<float>(SubItemRectangle.left) / static_cast<float>(DeviceContext.GetDeviceCaps(LOGPIXELSX));
					float UMax = static_cast<float>(SubItemRectangle.right) / static_cast<float>(DeviceContext.GetDeviceCaps(LOGPIXELSX));
