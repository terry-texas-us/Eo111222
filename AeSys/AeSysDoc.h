...
// Block Table interface
	CBlocks* BlocksTable() {
		return (&m_BlocksTable);
	}
	bool BlockTableIsEmpty() {
		return m_BlocksTable.IsEmpty() == TRUE;
	}
	EoUInt16 BlockTableSize() {
		return (EoUInt16(m_BlocksTable.GetSize()));
	}
	int GetBlockReferenceCount(const CString& name);
	POSITION GetFirstBlockPosition() {
		return m_BlocksTable.GetStartPosition();
	}
...
// Layer Table interface
	...
	int GetLayerTableSize() {
		return (int) m_LayerTable.GetSize();
	}
...
// Line Type Table interface
	EoDbLineTypeTable* LineTypeTable() {
		return &m_LineTypeTable;
	}
	EoDbLineType* ContinuousLineType() {
		return m_ContinuousLineType;
	}
...
public: // trap interface
	...
	EoGePoint3d GetTrapPivotPoint() {
		return m_TrapPivotPoint;
	}
	BOOL IsTrapEmpty() const {
		return m_TrappedGroupList.IsEmpty();
	}
	void ModifyTrappedGroupsPenColor(EoInt16 penColor) {
		m_TrappedGroupList.ModifyPenColor(penColor);
	}
	void ModifyTrappedGroupsLineType(EoInt16 lineType) {
		m_TrappedGroupList.ModifyLineType(lineType);
	}
	...
	void SetTrapPivotPoint(const EoGePoint3d& pt) {
		m_TrapPivotPoint = pt;
	}
