...
void ConvertTextData(OdDbText* text, EoDbGroup* group) {
	...
	EoDbFontDefinition FontDefinition(EoDb::kEoTrueType, (PCTSTR) FileName, EoDb::kPathRight, HorizontalAlignment, VerticalAlignment, 0.0);
...
void ConvertDimensionData(OdDbDimension* dimension) {
	...
	ATLTRACE2(traceOdDb, 2, L"Position: %f, %f, %f\n", dimension->dimBlockPosition()[0], dimension->dimBlockPosition()[1], dimension->dimBlockPosition()[2]);
	ATLTRACE2(traceOdDb, 2, L"Rotation: %f\n", dimension->dimBlockRotation());
	ATLTRACE2(traceOdDb, 2, L"Scale: %f, %f, %f\n", dimension->dimBlockScale()[0], dimension->dimBlockScale()[1], dimension->dimBlockScale()[2]);
	ATLTRACE2(traceOdDb, 2, L"Text Position: %f, %f, %f\n", dimension->textPosition()[0], dimension->textPosition()[1], dimension->textPosition()[2]);
	...
	ATLTRACE2(traceOdDb, 2, L"Normal: %f, %f, %f\n", dimension->normal()[0], dimension->normal()[1], dimension->normal()[2]);
};
...
void ConvertCurveData(OdDbEntity* entity, EoDbPrimitive* primitive) {
	...
	if (eOk == Curve->getStartPoint(StartPoint)) {
		ATLTRACE2(traceOdDb, 2, L"Start Point: %f, %f, %f\n", StartPoint);
	}
	OdGePoint3d EndPoint;
	if (eOk == Curve->getEndPoint(EndPoint)) {
		ATLTRACE2(traceOdDb, 2, L"End Point: %f, %f, %f\n", StartPoint);
	}
	if (Curve->isClosed())
		static_cast<EoDbPolyline*>(primitive)->SetFlag(EoDbPolyline::sm_Closed);

	ATLTRACE2(traceOdDb, 2, L"Periodic: %i\n", Curve->isPeriodic());

	double Area;
	if (eOk == Curve->getArea(Area)) {
		ATLTRACE2(traceOdDb, 2, L"Area: %f\n", Area);
	}
...
public:
	void Convert(OdDbEntity* entity, EoDbGroup* group) {
		...
		EoDbBlockReference* SegRefPrimitive = new EoDbBlockReference((PCTSTR) BlockTableRecordPtr->getName(), BlockReferenceEntity->position());
	...
	static void ConvertEdgesType(int loopIndex , OdDbHatchPtr &HatchEntity) {
		...
		for (int i = 0; i < (int) edges.size(); i++) {
			...
			case OdGe::kCircArc2d:
				ConvertCircularArcEdge(pEdge); break;
			case OdGe::kEllipArc2d:
				ConvertEllipticalArcEdge(pEdge); break;
...
public:
	void Convert(OdDbEntity* entity, EoDbGroup* group) {
		...
		EoDbBlockReference* SegRefPrimitive = new EoDbBlockReference((PCTSTR) BlockTableRecordPtr->getName(), MInsertBlockEntity->position());
...
public:
	void Convert(OdDbEntity* entity, EoDbGroup* group) {
		...
		case OdDb::kTopLeft:
			HorizontalAlignment = EoDb::kAlignLeft; VerticalAlignment = EoDb::kAlignTop; break;
		case OdDb::kTopCenter:
			HorizontalAlignment = EoDb::kAlignCenter; VerticalAlignment = EoDb::kAlignTop; break;
		case OdDb::kTopRight:
			HorizontalAlignment = EoDb::kAlignRight; VerticalAlignment = EoDb::kAlignTop; break;
		case OdDb::kMiddleLeft:
			HorizontalAlignment = EoDb::kAlignLeft; VerticalAlignment = EoDb::kAlignMiddle; break;
		case OdDb::kMiddleCenter:
			HorizontalAlignment = EoDb::kAlignCenter; VerticalAlignment = EoDb::kAlignMiddle; break;
		case OdDb::kMiddleRight:
			HorizontalAlignment = EoDb::kAlignRight; VerticalAlignment = EoDb::kAlignMiddle; break;
		case OdDb::kBottomCenter:
			HorizontalAlignment = EoDb::kAlignCenter; VerticalAlignment = EoDb::kAlignBottom; break;
		case OdDb::kBottomRight:
			HorizontalAlignment = EoDb::kAlignRight; VerticalAlignment = EoDb::kAlignBottom; break;
		default:
			HorizontalAlignment = EoDb::kAlignLeft; VerticalAlignment = EoDb::kAlignBottom;
		}
...
public:
	void Convert(OdDbEntity* entity, EoDbGroup* group) {
		...
		SplineEntity->getNurbsData(Degree, Rational, Closed, Periodic, ControlPoints, Knots, Weights, Tolerance, KTolerance);
...
public:
	void Convert(OdDbEntity* entity, EoDbGroup* group) {
		...
		EoDbCharacterCellDefinition ccd(TextEntity->rotation(), TextEntity->oblique(), TextEntity->widthFactor(), TextEntity->height());
	...
	void Convert(OdDbEntity* entity, EoDbGroup* /* group */) {
		...
		if (layerIds.length()) {
			...
			for (int i = 0; i < (int) layerIds.length(); i++) {
				ATLTRACE2(traceOdDb, 2, L"%i  ", layerIds[i]);
			}
		}
		else {
