...
public: // Methods - absolute virtuals
	...
	void Assign(EoDbPrimitive* primitive) {
		*this = *static_cast<EoDbPolyline*>(primitive);
	}
	...
	void SetFlag(const EoUInt16 w) {
		m_wFlags = w;
	}
	void SetPt(int index, const EoGePoint3d& pt) {
		m_pts[index] = pt;
	}
...
public:
	static EoUInt16& EdgeToEvaluate() {
		return sm_EdgeToEvaluate;
	}
	static EoUInt16& Edge() {
		return sm_Edge;
	}
	bool IsLooped() {
		return (m_wFlags != 0);
