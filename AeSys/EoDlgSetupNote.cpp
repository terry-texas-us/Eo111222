...
EoDlgSetupNote::EoDlgSetupNote(EoDbFontDefinition* fontDefinition, CWnd* pParent /*= NULL*/) :
	m_FontDefinition(fontDefinition), CDialog(EoDlgSetupNote::IDD, pParent), m_TextExpansionFactor(0), m_TextHeight(0), m_CharacterSlantAngle(0), m_TextRotationAngle(0) {
}
...
void EoDlgSetupNote::OnOK() {
	...
	EoUInt16 HorizontalAlignment = EoUInt16(1 - IDC_TEXT_ALIGN_HOR_LEFT + GetCheckedRadioButton(IDC_TEXT_ALIGN_HOR_LEFT, IDC_TEXT_ALIGN_HOR_RIGHT));
	m_FontDefinition->HorizontalAlignment(HorizontalAlignment);

	EoUInt16 VerticalAlignment = EoUInt16(4 + IDC_TEXT_ALIGN_VER_BOT - GetCheckedRadioButton(IDC_TEXT_ALIGN_VER_BOT, IDC_TEXT_ALIGN_VER_TOP));
	...
	if (FontsIndex != CB_ERR) {
		...
		EoUInt16 Precision = EoUInt16(FontsItemName.CompareNoCase(L"Simplex.psf") != 0 ? EoDb::kEoTrueType : EoDb::kStrokeType);
