...
void AeSysView::OnDraw2ModeOptions() {
	...
	if (dlg.DoModal() == IDOK) {
		m_DistanceBetweenLines = dlg.m_dLength;
	}
}
...
void AeSysView::OnDraw2ModeWall() {
	...
	if (m_EndSectionGroup == 0) {
		if (m_PreviousOp != 0) {
			...
			GetDocument()->UpdateAllViews(NULL, EoDb::kGroupSafe, m_AssemblyGroup);
		...
		GetDocument()->UpdateAllViews(NULL, EoDb::kGroupEraseSafe, m_EndSectionGroup);
		...
		GetDocument()->UpdateAllViews(NULL, EoDb::kGroupSafe, m_EndSectionGroup);
		m_EndSectionGroup = 0;
...
bool AeSysView::CleanPreviousLines() {
	...
	GetDocument()->UpdateAllViews(NULL, EoDb::kGroupEraseSafe, m_AssemblyGroup);
...
bool AeSysView::StartAssemblyFromLine() {
	...
	if (EoGeVector3d(Line.begin, m_CurrentLeftLine.begin).Length() > EoGeVector3d(Line.begin, m_CurrentRightLine.begin).Length()) {
