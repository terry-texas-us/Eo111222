...
// constructors
EoGeVector3d::EoGeVector3d() { x = y = z = 0.0; }
EoGeVector3d::EoGeVector3d(double initialX, double initialY, double initialZ) {
  x = initialX;
  y = initialY;
  z = initialZ;
}
...
bool EoGeVector3d::operator!=(const EoGeVector3d& vector) const { return (!EoGeNearEqual(*this, vector, FLT_EPSILON)); }
void EoGeVector3d::operator+=(const EoGeVector3d& vector) {
  x += vector.x;
  y += vector.y;
  z += vector.z;
}
...
EoGeVector3d RotateVectorAboutZAxis(const EoGeVector3d& vector, double angle) {
  ...
  if (fabs(angle) <= FLT_EPSILON || fabs(angle - TWOPI) <= FLT_EPSILON) {
    CosineAngle = 1.0;
  } else if (fabs(angle - HALF_PI) <= FLT_EPSILON || fabs(angle + PI + HALF_PI) <= FLT_EPSILON) {
    SinAngle = 1.0;
  } else if (fabs(angle - PI) <= FLT_EPSILON) {
    CosineAngle = -1.0;
  } else if (fabs(angle - PI - HALF_PI) <= FLT_EPSILON || fabs(angle + HALF_PI) <= FLT_EPSILON) {
    SinAngle = -1.0;
  } else {
    SinAngle = sin(angle);
