...
public: // Constructors and destructor
	...
	bool IsOpened() {
		return ((m_TracingFlgs & kTracingIsOpened) == kTracingIsOpened);
	}
	bool IsMapped() {
		return ((m_TracingFlgs & kTracingIsMapped) == kTracingIsMapped);
	}
	bool IsViewed() {
		return ((m_TracingFlgs & kTracingIsViewed) == kTracingIsViewed);
	}
	EoUInt16 GetTracingFlgs() {
		return m_TracingFlgs;
	}
	void SetTracingFlg(EoUInt16 w) {
		m_TracingFlgs = w;
	}

	void ClearStateFlag(EoUInt16 w = 0xffff) {
		m_StateFlags &= ~w;
	}
	EoUInt16 LayerStateFlags() const {
		return m_StateFlags;
	}
	bool IsActive() {
	...
	bool IsStatic() {
		return ((m_StateFlags & kIsStatic) == kIsStatic);
	}
	bool IsWork() {
		return ((m_StateFlags & kIsWork) == kIsWork);
	}
	void MakeInternal() {
