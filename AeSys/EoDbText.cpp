...
void EoDbText::AddToTreeViewControl(HWND hTree, HTREEITEM hParent) {
	tvAddItem(hTree, hParent, L"<Text>", this);
}
...
void EoDbText::AddReportToMessageList(EoGePoint3d) {
	CString str;
	str =
		L"Color: " + FormatPenColor() +
		L" Font: " + m_fd.FontName() +
		L" Precision: " + m_fd.FormatPrecision() +
		L" Path: " + m_fd.FormatPath() +
...
void EoDbText::FormatExtra(CString& str) {
	str.Format(L"Color;%s\tFont;%s\tPrecision;%s\tPath;%s\tAlignment;(%s,%s)\tSpacing;%f\tLength;%d\tText;%s", FormatPenColor(),
		m_fd.FontName(), m_fd.FormatPrecision(), m_fd.FormatPath(), m_fd.FormatHorizonatlAlignment(), m_fd.FormatVerticalAlignment(),
		m_fd.CharacterSpacing(), m_strText.GetLength(), m_strText);
}
...
EoGePoint3d EoDbText::GetCtrlPt() {
...
bool EoDbText::SelectUsingPoint(AeSysView* view, EoGePoint4d point, EoGePoint3d& ptProj) {
	...
	for (size_t n = 0; n < 4; n++) {
		if (EoGeLine(pt0[n], pt0[(n + 1) % 4]).DirRelOfPt(point) < 0)
			return false;
	}
	ptProj = point;
...
void EoDbText::Transform(EoGeTransformMatrix& tm) {
