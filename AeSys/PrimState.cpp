...
void CPrimState::SetPen(AeSysView* view, CDC* deviceContext, EoInt16 penColor, EoInt16 lineType) {
	if (EoDbPrimitive::SpecialPenColorIndex() != 0) {
		penColor = EoDbPrimitive::SpecialPenColorIndex();
	}
	if (penColor == EoDbPrimitive::PENCOLOR_BYLAYER) {
		penColor = EoDbPrimitive::LayerPenColorIndex();
	}
	if (EoDbPrimitive::SpecialLineTypeIndex() != 0) {
		lineType = EoDbPrimitive::SpecialLineTypeIndex();
	}
	...
	if (deviceContext) {
		ManagePenResources(deviceContext, penColor, int(LogicalWidth), lineType);
	}
}
void CPrimState::ManagePenResources(CDC* deviceContext, EoInt16 penColor, int penWidth, EoInt16 lineType) {
	...
	if (deviceContext) {
		deviceContext->SetTextColor(pColTbl[penColor]);
	}
...
void CPrimState::SetPenColor(CDC* deviceContext, EoInt16 penColor) {
	m_PenColor = penColor;
	if (deviceContext) {
		ManagePenResources(deviceContext, penColor, 0, m_LineType);
	}
}
void CPrimState::SetLineType(CDC* deviceContext, EoInt16 lineType) {
	m_LineType = lineType;
	if (deviceContext) {
		ManagePenResources(deviceContext, m_PenColor, 0, lineType);
	}
}
