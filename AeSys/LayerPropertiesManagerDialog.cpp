...
#include "LayerPropertiesManagerDialog.h"
...
// LayerPropertiesManagerDialog dialog

IMPLEMENT_DYNAMIC(LayerPropertiesManagerDialog, CDialog)

BEGIN_MESSAGE_MAP(LayerPropertiesManagerDialog, CDialog)
	ON_WM_CREATE()
	ON_WM_DRAWITEM()
	ON_WM_SIZE()
	ON_WM_SIZING()
	ON_BN_CLICKED(IDC_BUTTON_ADD, &LayerPropertiesManagerDialog::OnBnClickedButtonAdd)
	ON_BN_CLICKED(IDC_BUTTON_CURRENT, &LayerPropertiesManagerDialog::OnBnClickedButtonCurrent)
	ON_NOTIFY(NM_DBLCLK, IDC_LAYER_FILTER_TREE, &LayerPropertiesManagerDialog::OnNMDblclkLayerFilterTree)
	ON_NOTIFY(TVN_KEYDOWN, IDC_LAYER_FILTER_TREE, &LayerPropertiesManagerDialog::OnTvnKeydownLayerFilterTree)
	ON_NOTIFY(NM_CLICK, IDC_LAYERS_LIST_CONTROL, &LayerPropertiesManagerDialog::OnNMClickListLayersList)
	ON_NOTIFY(NM_DBLCLK, IDC_LAYERS_LIST_CONTROL, &LayerPropertiesManagerDialog::OnNMDblclkListLayersList)
	ON_NOTIFY(NM_RCLICK, IDC_LAYERS_LIST_CONTROL, &LayerPropertiesManagerDialog::OnNMRClickListLayersList)
END_MESSAGE_MAP()

LayerPropertiesManagerDialog::LayerPropertiesManagerDialog(CWnd* parent /*=NULL*/)
: CDialog(LayerPropertiesManagerDialog::IDD, parent)
{
}
LayerPropertiesManagerDialog::LayerPropertiesManagerDialog(OdDbDatabase* database, CWnd* parent /*=NULL*/)
: CDialog(LayerPropertiesManagerDialog::IDD, parent), m_Database(database)
{
}

LayerPropertiesManagerDialog::~LayerPropertiesManagerDialog()
{
}

void LayerPropertiesManagerDialog::DoDataExchange(CDataExchange* pDX)
{
...
// LayerPropertiesManagerDialog message handlers

int LayerPropertiesManagerDialog::OnCreate(LPCREATESTRUCT createStructure)
{
...
void LayerPropertiesManagerDialog::OnDrawItem(int controlIdentifier, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
void LayerPropertiesManagerDialog::OnSize(UINT type, int newWidth, int newHeight)
{
...
void LayerPropertiesManagerDialog::OnSizing(UINT side, LPRECT rectangle)
{
...
void LayerPropertiesManagerDialog::OnBnClickedButtonAdd()
{
	...
	do
	{
		Name.format(OD_T("Layer%d"), Suffix++);
	}
...
void LayerPropertiesManagerDialog::OnBnClickedButtonCurrent()
{
...
void LayerPropertiesManagerDialog::OnNMDblclkLayerFilterTree(NMHDR* /* pNMHDR */, LRESULT* pResult)
{
...
void LayerPropertiesManagerDialog::OnTvnKeydownLayerFilterTree(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void LayerPropertiesManagerDialog::OnNMClickListLayersList(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case column_descr:
		...
		{
			OdDbViewportPtr pVp = OdDbViewport::cast(Layer->database()->
	...
	case column_vplineweight:
		{
			...
			if (IDOK == dlg.DoModal())
			{
				Layer->setLineWeight(dlg.m_LineWeight, m_ActiveViewport);
			}
			break;
		}
...
void LayerPropertiesManagerDialog::OnNMDblclkListLayersList(NMHDR* /* pNMHDR */, LRESULT* pResult)
{
	if (m_ClickToColumnName)
	{
		OnBnClickedButtonCurrent();    
	}
	*pResult = 0;
}

void LayerPropertiesManagerDialog::OnNMRClickListLayersList(NMHDR* /* pNMHDR */, LRESULT* pResult)
{
...
BOOL LayerPropertiesManagerDialog::OnInitDialog(void)
{
...
bool LayerPropertiesManagerDialog::OpenSelectedLayer(OdSmartPtr<OdDbLayerTableRecord>& layer) 
{
...
void LayerPropertiesManagerDialog::UpdateCurrentLayerInfoField()
{
	CString Text;
...
void LayerPropertiesManagerDialog::UpdateFiltersTree()
{
	...
	if (FilterManager->getFilters(m_pRootFilter, pCurrent) != eOk )
	{
		return;
	}
...
void DrawColorBox(CDC& deviceContext, const RECT& itemRectangle, const OdCmColor& color) 
{
	...
	if(ItemRectangle.left <= itemRectangle.right)
	{
		...
		deviceContext.ExtTextOutW(ItemRectangle.left, ItemRectangle.top, ETO_CLIPPED, &itemRectangle, TextOut, TextOut.getLength(), NULL);
	}
}
void DrawLineWeight(CDC& deviceContext, const RECT& itemRectangle, const OdDb::LineWeight lineWeight) 
{
	float PixelsPerLogicalMillimeter = float(deviceContext.GetDeviceCaps(LOGPIXELSY) / EoMmPerInch);
...
void DrawPlotStyle(CDC& deviceContext, const RECT& itemRectangle, const CString& textOut, const OdDbDatabase* database) 
{
...
void LayerPropertiesManagerDialog::DrawItem(CDC& deviceContext, int itemID, int labelIndex, const RECT& itemRectangle) 
{
	CString TextOut;
	...
	case column_plotstyle:
		...
		break;
	case column_plot:
		m_stateImages.Draw(&deviceContext, Layer->isPlottable() ? 6 : 7, ((CRect&) itemRectangle).TopLeft(), ILD_TRANSPARENT);
		break;
	case column_descr:
	case column_vpfreeze:
