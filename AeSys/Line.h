...
public:	// Constructors and destructor
	...
	CLine(const EoGePoint3d& beginPoint, const EoGePoint3d& endPoint) 
...
public:	// Operators
	...
	{
		return (Identical(line, FLT_EPSILON));
	}
	bool operator!=(const CLine& line)
	{
		return (!Identical(line, FLT_EPSILON));
	...
	EoGePoint3d& operator[](int i) 
	{
		return (i == 0 ? begin : end);
	}
	const EoGePoint3d& operator[](int i) const
	{
		return (i == 0 ? begin : end);
	}
...
public: // Methods
	...
	EoUInt16 CutAtPt(EoGePoint3d&, CLine&);
	...
	/// </param>
	bool GetParallels(double dDis, double eccentricity, CLine& leftLine, CLine& rightLine);
	bool Identical(const CLine& line, double tolerance);
	...
	double Length();
	EoGePoint3d	Midpoint() 
	{
		return ProjectBeginPointToEndPoint(.5);
	}
	/// <summary>Determines if lines are parallel.</summary>
	bool ParallelTo(const CLine& line);
	...
	static double AngleBetweenLn_xy(CLine ln1, CLine ln2)	
	{
		EoGeVector3d v1(ln1.begin, ln1.end); v1.z = 0.;
		EoGeVector3d v2(ln2.begin, ln2.end); v2.z = 0.;
	...
	static EoGePoint4d IntersectionWithPln4(EoGePoint4d& beginPoint, EoGePoint4d& endPoint, const EoGePoint4d& pointOnPlane, EoGeVector3d& planeNormal)
	{
	...
	static bool IntersectionWithPln(EoGePoint3d& beginPoint, EoGeVector3d lineVector, EoGePoint3d pointOnPlane, EoGeVector3d planeNormal, EoGePoint3d* intersection)
	{	
		double dDotProd = planeNormal.DotProduct(lineVector);
		...
		if (fabs(dDotProd) > DBL_EPSILON)								// Line and plane are not parallel
		{
			...
			double dD = - planeNormal.DotProduct(vOnPln);				// Coefficient of plane on which point lies
			double dT = - (planeNormal.DotProduct(vEnd) + dD) / dDotProd;		// Parameter on the line where it intersects the plane
	...
	static int Intersection(CLine ln1, CLine ln2, EoGePoint3d& ptInt)
	{
		EoGeVector3d v1(ln1.begin, ln1.end);
		if (v1.IsNull(DBL_EPSILON))		// Endpoints of first line coincide
			return (FALSE);

		EoGeVector3d v2(ln2.begin, ln2.end);
		if (v2.IsNull(DBL_EPSILON))			// Endpoints of second line coincide
			return (FALSE);

		EoGeVector3d vPlnNorm = v1.CrossProduct(v2);
		vPlnNorm.Normalize();
		if (vPlnNorm.IsNull(DBL_EPSILON))			// Two lines are parallel
			return (FALSE);

		EoGeVector3d v3(ln1.begin, ln2.begin);
		if (fabs(vPlnNorm.DotProduct(v3)) > DBL_EPSILON) // Four points are not coplanar
			return (FALSE);

		CTMat tm(ln1.begin, vPlnNorm);
		...
		CLine Line1 = CLine(rL1P1, rL1P2);
		if (CLine::Intersection_xy(Line1, CLine(rL2P1, rL2P2), ptInt)) 
		{
			ptInt.z = 0.;
			tm.Inverse();
			ptInt = tm * ptInt;
	...
	static bool Intersection_xy(CLine ln1, CLine ln2, EoGePoint3d& ptInt)
	{	
