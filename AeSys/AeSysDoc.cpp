...
void AeSysDoc::DeleteContents() {
	ATLTRACE2(atlTraceGeneral, 1, L"AeSysDoc<%08.8lx>)::DeleteContents()\n", this);
...
BOOL AeSysDoc::DoSave(LPCWSTR pathName, BOOL replace) {
	...
	if (PathName.IsEmpty()) {
		...
		if (replace && PathName.IsEmpty()) {
			...
			if (FirstBadCharacterIndex != - 1) {
				PathName.ReleaseBuffer(FirstBadCharacterIndex);
			}
			CString Extension;
...
#if defined (USING_ODA)
		if (!DoPromptFileName(PathName, replace ? AFX_IDS_SAVEFILE : AFX_IDS_SAVEFILECOPY, OFN_HIDEREADONLY | OFN_EXPLORER | OFN_PATHMUSTEXIST)) {
			return FALSE;
		}
#endif // USING_ODA
	}
	if (!OnSaveDocument(PathName)) {
		if (pathName == NULL) {
			...
			CATCH_ALL(e) {
				ATLTRACE2(atlTraceGeneral, 0, L"Warning: failed to delete file after failed SaveAs.\n");
				do {
					e->Delete();
				} while (0);
			}
...
#else // Not USING_ODA
	m_LineTypeTable.SetAt(L"0", new EoDbLineType(0, L"Null", L"null", 0, NULL));
	EoDbLineType* LineType = new EoDbLineType(1, L"Continuous", L"Solid line", 0, NULL);
...
#endif // USING_ODA
	m_LineTypeTable.Lookup(L"Continuous", m_ContinuousLineType); // Assumes both creation paths create a "Continuous" LineType
...
BOOL AeSysDoc::OnOpenDocument(LPCWSTR lpszPathName) {
	ATLTRACE2(atlTraceGeneral, 0, L"AeSysDoc<%08.8lx>::OnOpenDocument(%s)\n", this, lpszPathName);
	...
	case EoDb::kPeg: {
			...
			m_DatabasePtr = app.createDatabase(true, OdDb::kEnglish);
#endif // USING_ODA
			EoDbLineType* LineType = new EoDbLineType(0, L"Null", L"null", 0, NULL);
			m_LineTypeTable.SetAt(L"0", LineType);
			LineType = new EoDbLineType(1, L"Continuous", L"Solid line", 0, NULL);
			m_LineTypeTable.SetAt(L"Continuous", LineType);
			m_WorkLayer = new EoDbLayer(L"0", EoDbLayer::kIsResident | EoDbLayer::kIsInternal | EoDbLayer::kIsActive, LineType);
...
void AeSysDoc::DeletedGroupsRestore() {
	if (!DeletedGroupsIsEmpty()) {
		...
		AddWorkLayerGroup(Group);
		UpdateAllViews(NULL, EoDb::kGroupSafe, Group);
	}
}
void AeSysDoc::GetExtents(AeSysView* view, EoGePoint3d& ptMin, EoGePoint3d& ptMax, EoGeTransformMatrix& tm) {
	...
	for (EoUInt16 w = 0; w < GetLayerTableSize(); w++) {
		...
		if (!Layer->IsOff()) {
			Layer->GetExtents(view, ptMin, ptMax, tm);
		}
	}
}
...
void AeSysDoc::DisplayAllLayers(AeSysView* view, CDC* deviceContext) {
	ATLTRACE2(atlTraceGeneral, 1, L"AeSysDoc<%08.8lx>::DisplayAllLayers(%08.8lx, %08.8lx)\n", this, view, deviceContext);
...
EoDbLayer* AeSysDoc::GetLayerTableLayerAt(int index) {
	return (index >= (int) m_LayerTable.GetSize() ? (EoDbLayer*) NULL : m_LayerTable.GetAt(index));
}
int AeSysDoc::FindLayerTableLayer(const CString& layerName) const {
	for (EoUInt16 w = 0; w < m_LayerTable.GetSize(); w++) {
		...
		if (layerName.CompareNoCase(Layer->Name()) == 0) {
			return (w);
		}
	}
	return (- 1);
...
void AeSysDoc::LayerBlank(const CString& name) {
	...
	if (Layer == 0) {
		...
		if (app.ConfirmMessageBox(IDS_MSG_CONFIRM_BLANK, name) == IDYES) {
			...
			UpdateAllViews(NULL, EoDb::kLayerErase, Layer);
...
HRESULT WriteAttributes(IXmlReader* reader) {
	...
	if (S_OK != hr) {
		ATLTRACE2(atlTraceGeneral, 0, L"Error moving to first attribute, error is %08.8lx", hr);
		return -1;
	}
	else {
		const WCHAR* Prefix;
		...
		while (TRUE) {
			if (!reader->IsDefault()) {
				...
				if (FAILED(hr = reader->GetPrefix(&Prefix, &PrefixLength))) {
					ATLTRACE2(atlTraceGeneral, 0, L"Error getting prefix, error is %08.8lx", hr);
					return -1;
				}
				if (FAILED(hr = reader->GetLocalName(&LocalName, NULL))) {
					ATLTRACE2(atlTraceGeneral, 0, L"Error getting local name, error is %08.8lx", hr);
					return -1;
				}
				if (FAILED(hr = reader->GetValue(&Value, NULL))) {
					ATLTRACE2(atlTraceGeneral, 0, L"Error getting value, error is %08.8lx", hr);
					return -1;
				}
				if (PrefixLength > 0) {
					ATLTRACE2(atlTraceGeneral, 0, L"Attr: %s:%s=\"%s\" \n", CString(Prefix), CString(LocalName), CString(Value));
				}
				else {
					ATLTRACE2(atlTraceGeneral, 0, L"Attr: %s=\"%s\" \n", CString(LocalName), CString(Value));
				}
			}
...
void AeSysDoc::LoadLineTypesFromXmlFile(const CString& pathName) {
	...
	if (FAILED(hr = SHCreateStreamOnFile(pathName, STGM_READ, &FileStream))) {
		ATLTRACE2(atlTraceGeneral, 0, L"Error creating file reader, error is %08.8lx", hr);
		return;
	}
	if (FAILED(hr = CreateXmlReader(__uuidof(IXmlReader), (void**) &Reader, NULL))) {
		ATLTRACE2(atlTraceGeneral, 0, L"Error creating xml reader, error is %08.8lx", hr);
		return;
	}
	if (FAILED(hr = Reader->SetProperty(XmlReaderProperty_DtdProcessing, DtdProcessing_Prohibit))) {
		ATLTRACE2(atlTraceGeneral, 0, L"Error setting XmlReaderProperty_DtdProcessing, error is %08.8lx", hr);
		return;
	}
	if (FAILED(hr = Reader->SetInput(FileStream))) {
		ATLTRACE2(atlTraceGeneral, 0, L"Error setting input for reader, error is %08.8lx", hr);
		return;
	}
	...
	while (S_OK == (hr = Reader->Read(&NodeType))) {
		...
		case XmlNodeType_XmlDeclaration:
			ATLTRACE2(atlTraceGeneral, 0, L"XmlDeclaration\n");
			if (FAILED(hr = WriteAttributes(Reader))) {
				ATLTRACE2(atlTraceGeneral, 0, L"Error writing attributes, error is %08.8lx", hr);
				return;
			}
			break;
		...
		case XmlNodeType_Element:
			...
			if (PrefixLength > 0) {
				ATLTRACE2(atlTraceGeneral, 0, L"Element: %s:%s\n", CString(Prefix), CString(LocalName));
			}
			else {
				ATLTRACE2(atlTraceGeneral, 0, L"Element: %s\n", CString(LocalName));
			}
			if (FAILED(hr = WriteAttributes(Reader))) {
				ATLTRACE2(atlTraceGeneral, 0, L"Error writing attributes, error is %08.8lx", hr);
				return;
			...
			if (FAILED(hr = Reader->MoveToElement())) {
				ATLTRACE2(atlTraceGeneral, 0, L"Error moving to the element that owns the current attribute node, error is %08.8lx", hr);
				return;
			}
			if (Reader->IsEmptyElement()) {
				ATLTRACE2(atlTraceGeneral, 0, L" (empty element)\n");
			}
			break;
		...
		case XmlNodeType_EndElement:
			...
			if (PrefixLength > 0) {
				ATLTRACE2(atlTraceGeneral, 0, L"End Element: %s:%s\n", CString(Prefix), CString(LocalName));
			}
			else {
				ATLTRACE2(atlTraceGeneral, 0, L"End Element: %s\n", CString(LocalName));
			}
			break;
		...
		case XmlNodeType_Text:
			...
			ATLTRACE2(atlTraceGeneral, 0, L"Text: >%s<\n", CString(Value));
			break;
		...
		case XmlNodeType_Whitespace:
			...
			ATLTRACE2(atlTraceGeneral, 1, L"Whitespace text: >%s<\n", CString(Value));
			break;
		...
		case XmlNodeType_CDATA:
			...
			ATLTRACE2(atlTraceGeneral, 0, L"CDATA: %s\n", Value);
			break;

		case XmlNodeType_ProcessingInstruction:
			if (FAILED(hr = Reader->GetLocalName(&LocalName, NULL))) {
				ATLTRACE2(atlTraceGeneral, 0, L"Error getting name, error is %08.8lx", hr);
				return;
			}
			if (FAILED(hr = Reader->GetValue(&Value, NULL))) {
				ATLTRACE2(atlTraceGeneral, 0, L"Error getting value, error is %08.8lx", hr);
			...
			ATLTRACE2(atlTraceGeneral, 0, L"Processing Instruction name:%S value:%S\n", CString(LocalName), CString(Value));
			break;
		...
		case XmlNodeType_Comment:
			...
			ATLTRACE2(atlTraceGeneral, 0, L"Comment: %s\n", CString(Value));
			break;
...
EoDbLayer* AeSysDoc::LayersSelUsingPoint(const EoGePoint3d& pt) {
	...
	for (EoUInt16 w = 0; w < GetLayerTableSize(); w++) {
		...
		if (Layer->SelectGroupUsingPoint(pt) != 0) {
			return (Layer);
		}
	}
	return 0;
...
bool AeSysDoc::TracingMap(const CString& pathName) {
	...
	if (bFileOpen) {
		Layer->SetTracingFlg(EoDbLayer::kTracingIsMapped);
		UpdateAllViews(NULL, EoDb::kLayerSafe, Layer);
	}
	return (bFileOpen);
}
...
void AeSysDoc::OnEditSegToWork() {
	...
	if (Layer != 0) {
		if (Layer->IsInternal()) {
			...
			if (Group != 0) {
				Layer->Remove(Group);
				UpdateAllViews(NULL, EoDb::kGroupEraseSafe, Group);
				AddWorkLayerGroup(Group);
				UpdateAllViews(NULL, EoDb::kGroup, Group);
			}
		}
...
void AeSysDoc::OnSetupFillSolid() {
	pstate.SetPolygonIntStyle(EoDb::kSolid);
}
...
void AeSysDoc::OnToolsPrimitiveDelete() {
	...
	if (Group != 0) {
		...
		UpdateAllViews(NULL, lHint, Group);
...
void AeSysDoc::OnPrimModifyAttributes() {
	...
	if (Group != 0) {
		...
		UpdateAllViews(NULL, EoDb::kPrimitiveSafe, ActiveView->EngagedPrimitive());
	}
}
...
void AeSysDoc::OnHelpKey() {
	...
	case ID_MODE_DRAW:
		WinHelpW(app.GetSafeHwnd(), L"peg.hlp", HELP_KEY, (DWORD) (LPWSTR) L"DRAW");
		break;

	case ID_MODE_EDIT: {
			WCHAR szKey[] = L"EDIT";
			WinHelpW(app.GetSafeHwnd(), L"peg.hlp", HELP_KEY, (DWORD) (LPWSTR) szKey);
			break;
		}
	case ID_MODE_TRAP:
	case ID_MODE_TRAPR: {
	...
	if (app.m_NodalModeAddGroups) {
		if (!btest(mask, bit)) {
			...
			if (AddUniquePoint(point) == 1) {
				...
				UpdateAllViews(NULL, EoDb::kPrimitiveEraseSafe, &PointPrimitive);
			}
		}
	}
	else {
...
void AeSysDoc::ConvertLayerTable() {
	for (int n = 0; n < GetLayerTableSize(); n++) {
		ConvertLayerTableRecord(GetLayerTableLayerAt(n));
	}
}
void AeSysDoc::ConvertLayerTableRecord(EoDbLayer* layer) {
	...
	if (Layers->getAt(OdString(layer->Name())).isNull()) {
		...
		Layers->add(Layer);
		if (layer->IsWork()) {
			m_DatabasePtr->setCLAYER(Layer->objectId());
		}
...
BOOL AeSysDoc::DoPromptFileName(CString& fileName, UINT titleResourceIdentifier, DWORD flags) {
	...
	if (DrawingVersion == OdDb::vAC24 && Extension.CompareNoCase(L"dwg") == 0) {
		FilterIndex = 6;
	}
	if (DrawingVersion == OdDb::vAC21 && Extension.CompareNoCase(L"dwg") == 0) {
		FilterIndex = 7;
	}
	if (fileName.Find('.') != - 1) {
	...
	CFileDialog FileDialog(FALSE, NULL, fileName.GetBuffer(_MAX_PATH), flags, Filter);
