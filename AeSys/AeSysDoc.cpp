...
int AeSysDoc::NumberOfGroupsInActiveLayers() {
  ...
  for (EoUInt16 w = 0; w < GetLayerTableSize(); w++) {
    EoDbLayer* Layer = GetLayerTableLayerAt(w);
    if (Layer->IsActive()) { iCount += Layer->GetCount(); }
  }
  return iCount;
}
  ...
  while (S_OK == (hr = Reader->Read(&NodeType))) {
    switch (NodeType) {
      ...
      case XmlNodeType_Element:
        ...
        if (Reader->IsEmptyElement()) { ATLTRACE2(atlTraceGeneral, 0, L" (empty element)\n"); }
        break;
...
int AeSysDoc::RemoveEmptyNotesAndDelete() {
  ...
  while (Position != NULL) { m_BlocksTable.GetNextAssoc(Position, Key, Block); }
  return (iCount);
}
...
void AeSysDoc::OnToolsGroupDelete() {
  ...
  if (Group != 0) {
    ...
    if (RemoveTrappedGroup(Group) != NULL) { ActiveView->UpdateStateInformation(AeSysView::TrapCount); }
    UpdateAllViews(NULL, EoDb::kGroupEraseSafe, Group);
