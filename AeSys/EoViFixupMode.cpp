...
void AeSysView::OnFixupModeReference(void) {
	...
	if (pSegRef != 0) {
		Document->UpdateAllViews(NULL, EoDb::kPrimitive, pPrimRef);
	}
	...
	if (!pPrimRef->Is(EoDb::kLinePrimitive)) {
		return;
	}
	ptCurPos = DetPt();
	...
	if (PreviousFixupCommand == 0)
		...
		if (PreviousFixupCommand == ID_OP2) {
			Document->UpdateAllViews(NULL, EoDb::kGroupEraseSafe, pSegPrv);
			...
			Document->UpdateAllViews(NULL, EoDb::kGroupSafe, pSegPrv);
		}
		else if (PreviousFixupCommand == ID_OP3) {
			if (EoGeVector3d(lnPrv.begin, ptInt).Length() < EoGeVector3d(lnPrv.end, ptInt).Length())
				lnPrv.begin = lnPrv.end;
			lnPrv.end = ptInt;
			if (EoGeVector3d(lnRef.end, ptInt).Length() < EoGeVector3d(lnRef.begin, ptInt).Length())
				lnRef.end = lnRef.begin;
			lnRef.begin = ptInt;
...
void AeSysView::OnFixupModeMend(void) {
	...
	if (PreviousFixupCommand == 0) {
		...
		Document->UpdateAllViews(NULL, EoDb::kGroupEraseSafe, pSegSec);
		...
		Document->UpdateAllViews(NULL, EoDb::kGroupSafe, pSegSec);
	}
	else {
		...
		if (PreviousFixupCommand == ID_OP2) {
			...
			if (EoGeVector3d(lnPrv.begin, ptInt).Length() < EoGeVector3d(lnPrv.end, ptInt).Length())
				lnPrv.begin = lnPrv.end;
			lnPrv.end = ptInt;
			if (EoGeVector3d(lnSec.end, ptInt).Length() < EoGeVector3d(lnSec.begin, ptInt).Length())
				lnSec.end = lnSec.begin;
			lnSec.begin = ptInt;
...
void AeSysView::OnFixupModeChamfer(void) {
	...
	if (PreviousFixupCommand == 0) {
		...
		if (EoGeVector3d(lnSec.end, ptInt).Length() < EoGeVector3d(lnSec.begin, ptInt).Length())
			lnSec.end = lnSec.begin;
		lnSec.begin = ptInt;
		if (pFndCPGivRadAnd4Pts(m_FixupModeCornerSize, lnPrv.begin, lnPrv.end, lnSec.begin, lnSec.end, &ptCP)) { // Center point is defined .. determine arc endpoints
			lnPrv.end = lnPrv.ProjPt(ptCP);
			lnSec.begin = lnSec.ProjPt(ptCP);
			if (PreviousFixupCommand == ID_OP1)
				;
