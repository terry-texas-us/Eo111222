...
: m_CurrentProgress(0), m_InProgress(false) {
...
int CMainFrame::OnCreate(LPCREATESTRUCT createStruct) {
	if (CMDIFrameWndEx::OnCreate(createStruct) == - 1)
		return - 1;
	...
	if (!m_StandardToolBar.CreateEx(this, TBSTYLE_FLAT, Style) || !m_StandardToolBar.LoadToolBar(app.HighColorMode() ? IDR_MAINFRAME_256 : IDR_MAINFRAME)) {
...
LRESULT CMainFrame::OnToolbarReset(WPARAM toolbarResourceId, LPARAM lparam) {
	ATLTRACE2(atlTraceGeneral, 1, L"CMainFrame::OnToolbarReset(%i, %i)\n", toolbarResourceId, lparam);
...
BOOL CMainFrame::OnShowPopupMenu(CMFCPopupMenu* pMenuPopup) {
	...
	if (pMenuPopup != NULL && pMenuPopup->GetMenuBar()->CommandToIndex(ID_VIEW_TOOLBARS) >= 0) {
		...
		if (PopupSubMenu) {
			pMenuPopup->GetMenuBar()->ImportFromMenu(*PopupSubMenu, TRUE);
		}
	}
	return TRUE;
}
void CMainFrame::UpdateMDITabs(BOOL resetMDIChild) {
	...
	case EoApOptions::Standard: {
			...
			EnableMDITabs(TRUE,
				app.m_Options.m_MdiTabInfo.m_bTabIcons,
				app.m_Options.m_MdiTabInfo.m_tabLocation,
				app.m_Options.m_MdiTabInfo.m_bTabCloseButton,
				app.m_Options.m_MdiTabInfo.m_style,
...
BOOL CMainFrame::OnShowMDITabContextMenu(CPoint point, DWORD dwAllowedItems, BOOL bDrop) {
	if (bDrop || !app.m_Options.m_bTabsContextMenu) {
		return FALSE;
	}
	CMenu menu;
...
BOOL CMainFrame::SetPaneText(int index, LPCWSTR newText) {
