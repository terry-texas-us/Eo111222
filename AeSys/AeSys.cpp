...
BOOL AeSys::InitInstance() {
	...
	if (FAILED(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, &m_Direct2dFactory))) {
		return FALSE;
	}
	...
	if (!RegisterKeyPlanWindowClass(m_hInstance)) {
		return FALSE;
	}
...
void AeSys::EditColorPalette() {
	...
	MessageBoxW(NULL, L"The background color is no longer associated with the pen Color Palette.", L"Deprecation Notice", MB_OK | MB_ICONINFORMATION);
...
void AeSys::FormatLength(CString& lengthAsString, Units units, const double length, const int width, const int precision) {
...
void AeSys::FormatLength_s(LPWSTR lengthAsString, const int bufSize, Units units, const double length, const int width, const int precision) {
	WCHAR szBuf[16];
