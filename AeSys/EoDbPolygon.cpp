...
EoDbPolygon::EoDbPolygon(EoInt16 penColor, EoInt16 style, EoInt16 styleIndex, EoGePoint3d& origin, EoGeVector3d& xAxis, EoGeVector3d& yAxis, EoGePoint3dArray& points)
	...
	for (EoUInt16 n = 0; n < m_NumberOfPoints; n++) {
		m_Pt[n] = points[n];
	}
}
...
void EoDbPolygon::AddToTreeViewControl(HWND hTree, HTREEITEM hParent) {
	tvAddItem(hTree, hParent, L"<Polygon>", this);
}
...
void EoDbPolygon::Display(AeSysView* view, CDC* deviceContext) {
	...
	if (m_InteriorStyle == EoDb::kHatch) {
		...
		for (EoUInt16 w = 0; w < m_NumberOfPoints; w++) {
			PointsArray[w] = EoGePoint4d(m_Pt[w]);
		}
...
void EoDbPolygon::FormatGeometry(CString& str) {
	...
	for (EoUInt16 w = 0; w < m_NumberOfPoints; w++) {
		str += L"Vertex Point;" + m_Pt[w].ToString();
	}
}
...
l1: 	dEps1 = DBL_EPSILON + DBL_EPSILON * fabs(dScan);
		...
		if (iEndEdg != iBegEdg) { // At least one pair of edge intersections .. generate scan lines for each pair
			...
			for (i = 1; i <= (iEndEdg - iBegEdg) / 2; i++) {
				...
				if (lnS.begin.x > edg[iCurEdg].dX)
					lnS.begin.x -= dTotStrLen;
				iStrId = 0;
