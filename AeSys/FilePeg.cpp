...
void CFilePeg::Load(AeSysDoc* document)
{
...
void CFilePeg::ReadHeaderSection(AeSysDoc*)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kHeaderSection)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kHeaderSection.";
	...
	if (EoDb::ReadUInt16(*this) != EoDb::kEndOfSection)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kEndOfSection.";
}
void CFilePeg::ReadTablesSection(AeSysDoc* document)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kTablesSection)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kTablesSection.";
...
void CFilePeg::ReadViewportTable(AeSysDoc*)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kViewPortTable)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kViewPortTable.";

	EoDb::ReadUInt16(*this);

	if (EoDb::ReadUInt16(*this) != EoDb::kEndOfTable)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kEndOfTable.";
}	
void CFilePeg::ReadLinetypesTable(AeSysDoc* document)
{
	EoDbLineTypeTable* LineTypeTable = document->LineTypeTable();

	if (EoDb::ReadUInt16(*this) != EoDb::kLinetypeTable)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kLinetypeTable.";
...
void CFilePeg::ReadLinetypeDefinition(double *dashLength, CString &name, CString &description, EoUInt16 &definitionLength)
{
	EoDb::Read(*this, name);
	...
	for (EoUInt16 n = 0; n < definitionLength; n++)
	{
		EoDb::Read(*this, dashLength[n]);
	}
}
void CFilePeg::ReadLayerTable(AeSysDoc* document)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kLayerTable)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kLayerTable.";
...
void CFilePeg::ReadBlocksSection(AeSysDoc* document)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kBlocksSection)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kBlocksSection.";
...
void CFilePeg::ReadEntitiesSection(AeSysDoc* document)
{
	if (EoDb::ReadUInt16(*this) != EoDb::kGroupsSection)
		throw L"Exception CFilePeg: Expecting sentinel EoDb::kGroupsSection.";
...
void CFilePeg::Unload(AeSysDoc* document)
{
	CFile::SetLength(0);
	CFile::SeekToBegin();
...
void CFilePeg::WriteHeaderSection(AeSysDoc*)
{
...
void CFilePeg::WriteTablesSection(AeSysDoc* document)
{
...
void CFilePeg::WriteLinetypeTable(AeSysDoc* document)
{
	...
	while(Position)
	{
		...
		if (DefinitionLength > 0)
		{
			...
			for (EoUInt16 w = 0; w < DefinitionLength; w++)
				EoDb::Write(*this, DashLength[w]);
...
void CFilePeg::WriteLayerTable(AeSysDoc* document)
{
...
void CFilePeg::WriteBlocksSection(AeSysDoc* document)
{
	...
	while (Position != 0)
	{
		...
		POSITION posPrim = Block->GetHeadPosition();
		while (posPrim != 0)
		{
			EoDbPrimitive* Primitive = Block->GetNext(posPrim);
...
void CFilePeg::WriteEntitiesSection(AeSysDoc* document)
{
...
bool EoDb::Read(CFile& file, EoDbPrimitive*& primitive)
{
	...
	case EoDbPrimitive::kPointPrimitive:
		...
		break;
	case EoDbPrimitive::kInsertPrimitive:
...
void EoDb::Read(CFile& file, EoInt16& number) 
{
...
void EoDb::Read(CFile& file, EoUInt16& number) 
{
...
void EoDb::Write(CFile& file, double number) 
{
	file.Write(&number, sizeof(double));
}
void EoDb::Write(CFile& file, EoInt16 number)
{
	file.Write(&number, sizeof(EoInt16));
}
void EoDb::Write(CFile& file, EoUInt16 number)
{
	file.Write(&number, sizeof(EoUInt16));
}
...
void EoDb::ConstructDimensionPrimitive(CFile& file, EoDbPrimitive*& primitive) 
{
	...
	CFontDef FontDefinition;
	FontDefinition.Read(file);
	CRefSys ReferenceSystem;
	ReferenceSystem.Read(file);
	CString Text;
	EoDb::Read(file, Text);

	primitive = new EoDbDimension(PenColor, LineType, CLine(BeginPoint, EndPoint),
		TextPenColor, FontDefinition, ReferenceSystem, Text);
}
...
void EoDb::ConstructPolygonPrimitive(CFile& file, EoDbPrimitive*& primitive)
{
	...
	{
		Points[n] = EoDb::ReadPoint3d(file);
	}
