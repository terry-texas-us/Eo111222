	...
	if (m_PreviousOp == 0) {
		...
		if (CorrectLeaderEndpoints(m_PreviousOp, ID_OP2, pts[0], CurrentPnt)) {
			...
			if (m_PreviousOp == ID_OP3) {
				GenerateLineEndItem(EndItemType(), EndItemSize(), CurrentPnt, pts[0], Group);
			}
...
void AeSysView::OnAnnotateModeBox() {
	...
	if (m_PreviousOp != ID_OP7) {
		...
		if (bG1Flg && bG2Flg) {
			...
			for (int i = 0; i < 4; i++)
				Group->AddTail(new EoDbLine(1, 1, pts[i], pts[(i + 1) % 4]));
...
void AeSysView::OnAnnotateModeCutIn() {
	...
	if (Group != 0) {
		...
		GetDocument()->UpdateAllViews(NULL, EoDb::kGroup, Group);
		pstate.Restore(DeviceContext, PrimitiveState);
	}
	ReleaseDC(DeviceContext);
}
...
bool AeSysView::CorrectLeaderEndpoints(int beginType, int endType, EoGePoint3d& beginPoint, EoGePoint3d& endPoint) {
	...
	if (LineSegmentLength > BeginDistance + EndDistance + DBL_EPSILON) {
		if (BeginDistance != 0.0)
			beginPoint = beginPoint.ProjectToward(endPoint, BeginDistance);
		if (EndDistance != 0.0)
			endPoint = endPoint.ProjectToward(beginPoint, EndDistance);
		return true;
	}
	else {
