...
#if defined(USING_ODA)
	, public ExSystemServices, public ExHostAppServices {
protected:
...
public:
	...
	UINT CheckMenuItem(UINT uId, UINT uCheck) {
	...
	float DeviceHeightInMillimeters() const {
		return m_DeviceHeightInMillimeters;
	}
	float DeviceHeightInPixels() const {
		return m_DeviceHeightInPixels;
	}
	float DeviceWidthInMillimeters() const {
		return m_DeviceWidthInMillimeters;
	}
	float DeviceWidthInPixels() const {
	...
	/// </summary>
	void FormatLength_s(LPWSTR lengthAsString, const int bufSize, Units units, const double length, const int width, const int precision);
	int	GetArchitecturalUnitsFractionPrecision() {
	...
	COLORREF PenColorsGetHot(EoInt16 nPenColor) {
		return (ColorPalette[nPenColor]);
	}
	void LoadPenColorsFromFile(const CString& pathName);
	double PenWidthsGet(EoInt16 nPenColor) {
		return (dPWids[nPenColor]);
	}
...
public:
	double ExtractedNumber() {
		return m_ExtractedNumber;
	}
	void SetExtractedNumber(const double number) {
		m_ExtractedNumber = number;
	}
	CString ExtractedString() {
		return m_ExtractedString;
	}
	void SetExtractedString(const CString& string) {
		m_ExtractedString = string;
	}
#endif // USING_DDE
protected:
	DECLARE_MESSAGE_MAP()
