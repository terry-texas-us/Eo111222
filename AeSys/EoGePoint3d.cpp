...
#endif // USING_ODA
bool EoGePoint3d::operator==(const EoGePoint3d& point) const {
	return (IsEqualTo(point, FLT_EPSILON));
}
bool EoGePoint3d::operator!=(const EoGePoint3d& point) const {
	return (!IsEqualTo(point, FLT_EPSILON));
}
...
EoGePoint3d	EoGePoint3d::operator*(double t) const {
	return EoGePoint3d(x * t, y * t, z * t);
}
EoGePoint3d	EoGePoint3d::operator/(double t) const {
	return EoGePoint3d(x / t, y / t, z / t);
}
...
bool EoGePoint3d::IsContained(const EoGePoint3d& lowerLeftPoint, const EoGePoint3d& upperRightPoint) const {
	...
	if (lowerLeftPoint.x > x + dReps || upperRightPoint.x < x - dReps)
		return false;

	dReps = DBL_EPSILON + fabs(DBL_EPSILON * y);
	if (lowerLeftPoint.y > y + dReps || upperRightPoint.y < y - dReps)
		return false;
...
EoGePoint3d EoGePoint3d::Mid(EoGePoint3d& ptA, EoGePoint3d& ptB) {
	return ptA + (ptA - ptB) * .5;
}
...
EoGePoint3d EoGePoint3d::RotateAboutAxis(const EoGePoint3d& referenceOrigin, const EoGeVector3d& referenceAxis, const double angle) {
	if (referenceAxis == EoGeVector3d::kZAxis) {
		...
		return (EoGePoint3d(
			referenceOrigin.x + (v.x * CosAng - v.y * SinAng),
			referenceOrigin.y + (v.x * SinAng + v.y * CosAng),
			z));
	}
	else {
