...
public:
	...
	POSITION GetStartPosition() {
		return m_MapLineTypes.GetStartPosition();
	}
	...
	void SetAt(const CString& name, EoDbLineType* lineType) {
		m_MapLineTypes.SetAt(name, lineType);
	}
	int ReferenceCount(EoInt16 lineType);
	int Size() {
		return (int) m_MapLineTypes.GetSize();
	}
