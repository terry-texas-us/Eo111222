	...
	if (!m_PowerArrow || (PointOnCircuit != CurrentPnt)) {
		...
		if (Group != 0) {
			...
			if (Circuit->RelOfPt(CurrentPnt) <= .5) {
				...
				if (CurrentPnt.DistanceTo(Circuit->BeginPoint()) <= .1)
					CurrentPnt = Circuit->BeginPoint();
			}
			else {
...
void AeSysView::GeneratePowerConductorSymbol(EoUInt16 conductorType, EoGePoint3d& pointOnCircuit, EoGePoint3d& endPoint) {
