...
EoDbEllipse::EoDbEllipse(EoInt16 penColor, EoInt16 lineType, EoGePoint3d& centerPoint, double radius)
	...
	m_vMajAx = m_vMinAx;
	m_vMajAx.RotAboutArbAx(PlaneNormal, - HALF_PI);
	m_dSwpAng = TWOPI;
}
EoDbEllipse::EoDbEllipse(EoInt16 penColor, EoInt16 lineType, EoGePoint3d& centerPoint, EoGeVector3d& planeNormal, double radius)
	...
	m_vMajAx *= radius;
	m_vMinAx = m_vMajAx;
	EoGeTransformMatrix tm(centerPoint, PlaneNormal, HALF_PI);
...
EoDbEllipse::EoDbEllipse(EoGePoint3d beginPoint, EoGePoint3d ptInt, EoGePoint3d endPoint) {
	...
	if (fabs(dDet) > DBL_EPSILON) { // Three points are not colinear
		...
		double dMin = EoMin(dAng[0], dAng[2]);
		double dMax = EoMax(dAng[0], dAng[2]);
...
void EoDbEllipse::AddToTreeViewControl(HWND hTree, HTREEITEM hParent) {
		tvAddItem(hTree, hParent, L"<Arc>", this);
}
...
void EoDbEllipse::CutAt2Pts(EoGePoint3d* pt, EoDbGroupList* groups, EoDbGroupList* newGroups) {
	...
	if (dRel[0] <= DBL_EPSILON && dRel[1] >= 1. - DBL_EPSILON) { // Put entire arc in trap
		...
		if (fabs(m_dSwpAng - TWOPI) <= DBL_EPSILON) { // Closed arc
...
void EoDbEllipse::AddReportToMessageList(EoGePoint3d) {
	CString str;
	str.Format(L"<Ellipse> Color: %s Line Type: %s SweepAngle %f MajorAxisLength: %f",
		FormatPenColor(), FormatLineType(), m_dSwpAng, m_vMajAx.Length());
...
void EoDbEllipse::FormatExtra(CString& str) {
	str.Format(L"Color;%s\tStyle;%s\tSweep Angle;%f\tMajor Axis Length;%f",
		FormatPenColor(), FormatLineType(), m_dSwpAng, m_vMajAx.Length());
}
EoGePoint3d EoDbEllipse::GetBegPt() {
	return (m_ptCenter + m_vMajAx);
}
void EoDbEllipse::GetBoundingBox(EoGePoint3dArray& ptsBox) {
	ptsBox.SetSize(4);
	ptsBox[0] = EoGePoint3d(- 1., - 1., 0.0);
	ptsBox[1] = EoGePoint3d(1., - 1., 0.0);
	ptsBox[2] = EoGePoint3d(1., 1., 0.0);
	ptsBox[3] = EoGePoint3d(- 1., 1., 0.0);
...
int EoDbEllipse::IsWithinArea(EoGePoint3d ptLL, EoGePoint3d ptUR, EoGePoint3d* ptInt) {
	...
	if (fabs(m_dSwpAng - TWOPI) <= DBL_EPSILON) { // Arc is a circle in disuise
...
bool pFndCPGivRadAnd4Pts(double adRad, EoGePoint3d arLn1Beg, EoGePoint3d arLn1End, EoGePoint3d arLn2Beg, EoGePoint3d arLn2End, EoGePoint3d* centerPoint) {
	...
	dSgnRad = (arLn1End.x * arLn2End.y - arLn2End.x * arLn1End.y) >= 0. ? - fabs(adRad) : fabs(adRad);
...
int pFndSwpAngGivPlnAnd3Lns(EoGeVector3d planeNormal, EoGePoint3d arP1, EoGePoint3d arP2, EoGePoint3d arP3, EoGePoint3d& centerPoint, double *adTheta) {
	...
	if (arP1 == centerPoint || arP2 == centerPoint || arP3 == centerPoint)
		return (FALSE);
	...
	double dTMin = EoMin(dT[0], dT[2]);
	double dTMax = EoMax(dT[0], dT[2]);
