...
EoDbLineType::EoDbLineType(EoUInt16 index, const CString& name, const CString& description, EoUInt16 numberOfDashElements, double* dashLengths) {
	m_Index = index;
	m_Name = name;
	...
	if (m_NumberOfDashElements == 0) {
		...
		for (int i = 0; i < m_NumberOfDashElements; i++) {
			m_DashElements[i] = EoMax(- 99., EoMin(dashLengths[i], 99.));
		}
	}
}
EoDbLineType::EoDbLineType(const EoDbLineType& lineType) {
	...
	m_DashElements = new double[m_NumberOfDashElements];
	for (int i = 0; i < m_NumberOfDashElements; i++) {
		m_DashElements[i] = lineType.m_DashElements[i];
	}
}
EoDbLineType::~EoDbLineType() {
	delete [] m_DashElements;
}
