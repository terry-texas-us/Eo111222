...
EoDbLineType::EoDbLineType(std::int16_t index, const CString& name, const CString& description, EoUInt16 numberOfDashElements,
                           double* dashLengths)
    : m_Index(index), m_Name(name), m_Description(description) {
  m_DashElements.reserve(numberOfDashElements);
  for (EoUInt16 i = 0; i < numberOfDashElements; i++) { m_DashElements.push_back(std::clamp(dashLengths[i], -99.0, 99.0)); }
}
