...
CLine CLine::operator-(EoGeVector3d v)
{
	return (CLine(begin - v, end - v));
} 
CLine CLine::operator+(EoGeVector3d v)
{
	return (CLine(begin + v, end + v));
}
double CLine::AngleFromXAxisXY()
{
...
EoGePoint3d CLine::ConstrainToAxis(double dInfAng, double dAxOffAng)
{	
	CTMat tm;
	tm.Translate(EoGeVector3d(begin, EoGePoint3d::kOrigin));
	
	CTMat tmZRot;
	tm *= tmZRot.RotateZ(- sin(EoToRadian(dAxOffAng)), cos(EoToRadian(dAxOffAng)));
	...
	{	// Not a zero length line
		...
		{	// Major component of line is along x-axis
			...
			if (dLen > DBL_EPSILON) 				// Not already on the x-axis
				if (dLen / fabs(pt.x) < tan(EoToRadian(dInfAng))) 
		...
		{	// Major component of line is along y-axis
			...
			if (dLen > DBL_EPSILON)					// Not already on the y-axis
				if (dLen / fabs(pt.y) < tan(EoToRadian(dInfAng))) 
		...
		else 
		{
			...
			if (dLen > DBL_EPSILON)					// Not already on the z-axis
				if (dLen / fabs(pt.z) < tan(EoToRadian(dInfAng))) 
...
EoUInt16 CLine::CutAtPt(EoGePoint3d& pt, CLine& ln)
{
	EoUInt16 wRet = 0;
...
int CLine::DirRelOfPt(EoGePoint3d pt)
{
...
void CLine::Display(AeSysView* view, CDC* deviceContext)
{
...
void CLine::Extents(EoGePoint3d& minExtent, EoGePoint3d& maxExtent)
{
...
bool CLine::Identical(const CLine& line, double tolerance) 
{
...
bool CLine::GetParallels(double distanceBetweenLines, double eccentricity, CLine& leftLine, CLine& rightLine)
{
	leftLine = *this;
	rightLine = *this;
	...
	if (LengthOfLines > FLT_EPSILON)
	{
		...
		leftLine += EoGeVector3d(- X * eccentricity, Y * eccentricity, 0.);
		rightLine += EoGeVector3d(X * (1. - eccentricity), - Y * (1. - eccentricity), 0.);

		return true;
	}
	return false;
}
bool CLine::IsContainedXY(const EoGePoint3d& lowerLeftPoint, const EoGePoint3d& upperRightPoint)
{
	EoGePoint3d	pt[2];
	pt[0] = begin; 
	pt[1] = end;
...
bool CLine::IsSelectedByPointXY(EoGePoint3d pt, const double apert, EoGePoint3d& ptProj, double* rel)
{
...
bool CLine::ParallelTo(const CLine& line)
{	
...
EoGePoint3d CLine::ProjPt(EoGePoint3d pt)
{
...
EoGePoint3d CLine::ProjectBeginPointToEndPoint(const double t)
{	
	return begin + (begin - end) * t;
}	 
int CLine::ProjPtFrom_xy(double parallelDistance, double perpendicularDistance, EoGePoint3d* projectedPoint)
{
	double dX = end.x - begin.x;
	double dY = end.y - begin.y;
...
EoGePoint3d CLine::ProjToBegPt(double dDis)
{
...
EoGePoint3d CLine::ProjToEndPt(double dDis)
{
	EoGeVector3d vBegEnd(begin, end);
...
void CLine::Read(CFile& file)
{
	begin.Read(file);
	end.Read(file);
}
bool CLine::RelOfPtToEndPts(EoGePoint3d pt, double& dRel)
{
