...
EoGeMatrixRow& EoGeMatrixRow::operator+(EoGeMatrixRow& row) {
	EoGeMatrixRow Row = *this;
	return Row += row;
}
...
EoGeMatrix::EoGeMatrix(const EoGeMatrixRow& v0, const EoGeMatrixRow& v1, const EoGeMatrixRow& v2, const EoGeMatrixRow& v3) {
	m_row[0] = v0;
	m_row[1] = v1;
	m_row[2] = v2;
	m_row[3] = v3;
...
EoGeMatrix& EoGeMatrix::Inverse() { // Gauss-Jordan elimination with partial pivoting
	...
	for (int iCol = 0; iCol < 4; iCol++) { // Find largest pivot in column iCol among rows iCol..3
		...
		for (i = iCol + 1; i < 4; i++) {
			if (fabs(mA.m_4X4[i][iCol]) > fabs(mA.m_4X4[i1][iCol])) {
				i1 = i;
			}
		}
