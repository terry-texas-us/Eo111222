...
EoDbLine::EoDbLine(EoGeLine& line)
	: m_ln(line) {
	m_PenColor = pstate.PenColor();
	m_LineType = pstate.LineType();
}
EoDbLine::EoDbLine(EoInt16 penColor, EoInt16 lineType, EoGeLine line)
	: EoDbPrimitive(penColor, lineType), m_ln(line) {
}
...
void EoDbLine::CutAtPt(EoGePoint3d& pt, EoDbGroup* group) {
	...
	if (m_ln.CutAtPt(pt, ln) != 0)
		group->AddTail(new EoDbLine(m_PenColor, m_LineType, ln));
}
...
void EoDbLine::AddReportToMessageList(EoGePoint3d pt) {
	...
	CString Message;
	Message.Format(L"<Line> Color: %s Line Type: %s - %s @ %s", FormatPenColor(), FormatLineType(), LengthAsString.TrimLeft(), AngleAsString);
...
void EoDbLine::FormatExtra(CString& str) {
	...
	str.Format(L"Color;%s\tStyle;%s\tLength;%s\tZ-Angle;%f", FormatPenColor(), FormatLineType(), FormattedLength.TrimLeft(), EoToDegree(m_ln.AngleFromXAxisXY()));
}
...
bool EoDbLine::Identical(EoDbPrimitive* primitive) {
	return m_ln == static_cast<EoDbLine*>(primitive)->Ln();
}
