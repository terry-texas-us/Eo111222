...
EoDbPolyline::EoDbPolyline(EoInt16 penColor, EoInt16 lineType, EoGePoint3d& centerPoint, double radius, int numberOfSides)
...
void EoDbPolyline::AddReportToMessageList(EoGePoint3d ptPic) {
	...
	if (sm_Edge > 0 && sm_Edge <= NumberOfVertices) {
		...
		CString Message;
		Message.Format(L"<Polyline Edge> Color: %s Line Type: %s - %s @ %s", FormatPenColor(), FormatLineType(), LengthAsString.TrimLeft(), AngleAsString);
