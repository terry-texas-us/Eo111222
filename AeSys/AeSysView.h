...
public:
	OdGsViewPtr ViewAt(int viewIndex) {
		return m_Device->viewAt(viewIndex);
	}
	/// <summary> Translates the camera position and target by the specified Camera Coordinate dolly amount. </summary>
	void Dolly(int x, int y);

	void ResetDevice(bool zoomExtents);
...
public:
	...
	afx_msg BOOL OnMouseWheel(UINT nFlags, EoInt16 zDelta, CPoint pt);
...
public: // Operations
	...
	float OverviewUExt() {
		return m_OverviewViewTransform.UExtent();
	}
	float OverviewUMin() {
		return m_OverviewViewTransform.UMin();
	}
	float OverviewVExt() {
		return m_OverviewViewTransform.VExtent();
	}
	float OverviewVMin() {
		return m_OverviewViewTransform.VMin();
	}
	CPoint DoProjection(const EoGePoint4d& pt) {
		return m_Viewport.DoProjection(pt);
	}
	...
	void DoProjection(CPoint* pnt, EoGePoint4dArray& pointsArray) {
		m_Viewport.DoProjection(pnt, pointsArray);
	}
	void DoProjectionInverse(EoGePoint3d& pt) {
		m_Viewport.DoProjectionInverse(pt);
	}
	...
	void SetDeviceHeightInInches(double height) {
		m_Viewport.SetDeviceHeightInInches(static_cast<float>(height));
	}
	void SetDeviceWidthInInches(double width) {
		m_Viewport.SetDeviceWidthInInches(static_cast<float>(width));
	}
...
public: // Edit mode interface
	...
	EoGeVector3d AeSysView::EditModeRotationAngles() {
	...
	EoGeVector3d AeSysView::EditModeInvertedScaleFactors() {
		...
		InvertedScaleFactors.y = fabs(m_EditModeScale.y) > DBL_EPSILON ? 1. / m_EditModeScale.y : 1.;
		InvertedScaleFactors.z = fabs(m_EditModeScale.z) > DBL_EPSILON ? 1. / m_EditModeScale.z : 1.;

		return InvertedScaleFactors;
	}
	EoGeVector3d AeSysView::EditModeScaleFactors() const {
		return m_EditModeScale;
	}
	...
	EoGeVector3d AeSysView::EditModeMirrorScale() const {
		return m_EditModeMirrorScale;
	}
...
public:
	...
	/// <param name="group"></param>
	void GenerateRectangularElbow(EoGeLine& previousReferenceLine, Section previousSection, EoGeLine& currentReferenceLine, Section currentSection, EoDbGroup* group);
	...
	/// <param name="group"></param>
	EoGePoint3d GenerateBullheadTee(EoDbGroup* existingGroup, EoGeLine& existingSectionReferenceLine, double existingSectionWidth, double existingSectionDepth, EoDbGroup* group);
	...
	void GenerateTransition(EoGeLine& referenceLine, double eccentricity, EJust justification, double slope, Section previousSection, Section currentSection, EoDbGroup* group);
