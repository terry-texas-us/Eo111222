...
class LexGen
{
    ...
    {
        if (lp >= line.Length)
            throw new Exception("Unexpected end after escape");
        ...
        case 'x':
            ...
            if (lp + 1 >= line.Length)
            {
                throw new Exception("Invalid hex escape sequence");
            }
        ...
        for (int i = 1; i <= d; i++)
        {
            ...
            for (int j = 1; j <= d; j++)
            {
                if ((i != j) && !used[i] && !changed[j])
                {
                    ...
                    {
                        same[k] =
                            ((dfanext[i, k] == 0 && dfanext[j, k] == 0) || (dfanext[i, k] != 0 && dfanext[j, k] != 0));
                        if (!same[k])
                        {
                            sameset = false;
                        }
    ...
    public static void mklex(out int nstates, out int nentries)
    {
        ...
        for (int s = 2; s <= d; s++)
        {
            ...
            {
                offset = 0;
                while (offset <= MAXCHAR && dfanext[s, offset] == 0)
                {
                    offset++;
                }
                iBase[s] = p - offset;
            ...
            for (int k = 1; k <= r; k++)
            {
                ...
                for (int i = 2; i <= n; i++)
                {
                    ...
                    while ((j < i - 1) && (!found))
                    {
                        ...
                        {
                            if (nextG[group[j], c] != nextG[group[i], c])
                            {
                                same = false;
                            }
                            c++;
    ...
    (int[] set1, int[] set2, ref int[] uset)
    {
        int i = 1;
        int j = 1;
